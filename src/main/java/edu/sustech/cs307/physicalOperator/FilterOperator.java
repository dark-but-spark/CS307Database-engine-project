 ...
 * 
 *   <li>hasNext() → 调用 findNext() 搜索下一个满足 WHERE 条件的元组
 *   <li>findNext() → 循环从 child 取元组，调用 tuple.eval_expr(whereExpr) 判断
 *   <li>Next() → 消费已准备好的元组，置 readyForNext=false
 * 
 *
 * 表达式求值（eval_expr）:
 *
 ...
 * 例如：WHERE age > 18 AND name = 'alice' 被解析为
 * AndExpression(GreaterThan(age, 18), EqualsTo(name, 'alice'))。
 *
 * 为什么索引扫描后仍保留 FilterOperator？（答辩高频追问）:
 *
 *
 * PhysicalPlanner 生成 FilterOperator(IndexScanOperator, whereExpr) 而非
 * 直接用 IndexScanOperator。因为：
 * 
 *   <li>索引可能只吸收部分 WHERE 条件（如只用 id 索引，name 条件未吸收）
 *   <li>FilterOperator 保证完整 WHERE 语义，避免 residual predicate 漏判
 *   <li>即使索引已完全覆盖所有条件，FilterOperator 也只多一次 eval 调用，无副作用
 * 
 */
