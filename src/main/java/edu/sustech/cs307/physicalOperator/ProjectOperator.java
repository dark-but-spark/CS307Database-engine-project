 ...
 * 
 *   <li>SELECT *：展开为用户请求的所有列（TabCol(tablename, colName)）→ 映射到 child schema
 *   <li>SELECT col1, col2：按列名 + 表名限定符匹配 child schema 中的列
 *   <li>多表同名列不写表名限定符 → 抛出 AmbiguousColumnName（防止歧义）
 * 
 *
 * ProjectTuple:
 *
 ...
 * values[i] = originalTuple.getValue(resolvedTabCols[i])。
