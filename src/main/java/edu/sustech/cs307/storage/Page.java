 ...
 *
 * 
 *   <li>DEFAULT_PAGE_SIZE = 4096（4KB）：与操作系统页大小对齐，磁盘 I/O 基本单位
 *   <li>PAGE_HEADER_SIZE = 8：页头保留字节（用于 record page 的 bitmap 等元数据）
 *   <li>data（ByteBuf）：Netty 字节缓冲区，存放实际页面内容
 *   <li>position（PagePosition）：页在磁盘上的位置（filename + offset）
 *   <li>dirty：脏标记。true 表示内存中的页已被修改，需要写回磁盘
 *   <li>pin_count：引用计数。>0 表示正在被使用（不可淘汰），0 表示可淘汰
 * 
 *
 * dirty 标记的语义:
 *
