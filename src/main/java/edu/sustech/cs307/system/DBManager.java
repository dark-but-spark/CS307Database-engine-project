...
public class DBManager {
    ...
    public void addColumn(String tableName, String columnName, String dataType) throws DBException {
        ...
        switch (dataType.toLowerCase()) {
            case "int" -> { type = ValueType.INTEGER; typeLen = Value.INT_SIZE; }
            case "float" -> { type = ValueType.FLOAT; typeLen = Value.FLOAT_SIZE; }
            case "char" -> { type = ValueType.CHAR; typeLen = Value.CHAR_SIZE; }
            default -> throw new DBException(ExceptionTypes.UnsupportedCommand("ALTER TABLE ADD " + dataType));
        }
        int offset = 0;
        for (ColumnMeta cm : tableMeta.columns_list) {
            offset += cm.len;
        }
        ColumnMeta newCol = new ColumnMeta(tableName, columnName, type, typeLen, offset);
        metaManager.addColumnInTable(tableName, newCol);
        metaManager.saveToJson();
        Logger.info("Column {} added to table {}", columnName, tableName);
    }
    ...
    public void renameTable(String oldName, String newName) throws DBException {
        if (!metaManager.getTableNames().contains(oldName)) {
            throw new DBException(ExceptionTypes.TableDoesNotExist(oldName));
        }
        if (metaManager.getTableNames().contains(newName)) {
            throw new DBException(ExceptionTypes.TableAlreadyExist(newName));
        }
        TableMeta tableMeta = metaManager.getTable(oldName);
        tableMeta.tableName = newName;
        for (ColumnMeta cm : tableMeta.columns_list) {
            cm.tableName = newName;
        }
        metaManager.dropTable(oldName);
        metaManager.createTable(tableMeta);
        String oldFolder = diskManager.getCurrentDir() + "/" + oldName;
        String newFolder = diskManager.getCurrentDir() + "/" + newName;
        new File(oldFolder).renameTo(new File(newFolder));
        Logger.info("Table {} renamed to {}", oldName, newName);
    }
